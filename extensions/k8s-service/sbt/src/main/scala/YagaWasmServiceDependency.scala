...
case class YagaWasmServiceDependency(
  ...
  private def infraCodegenTask(
    ...
    // image. The "correct" cluster path for wasm workloads is a containerd wasm shim
    // (e.g. `runwasi`'s `containerd-shim-wasmtime-v1`) + a `RuntimeClass` — that's a
    // separate initiative. For the yaga example / CI path, we build a small Debian-based
    // image that bakes the pinned wasmtime release in and runs it as a normal process.
    // Identical runtime semantics to the local Gate-1 `wasmtime serve` command.
    //
